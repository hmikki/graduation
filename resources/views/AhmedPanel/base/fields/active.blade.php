...
<div class="form-group label-floating">
        ...
        <select name="{{$Field['name']}}" style="margin: 0;padding: 0" id="{{$Field['name']}}" class="form-control">
            <option value="1" @if($value == '1') selected @endif>{{__('dashboard.activation.active')}}</option>
            <option value="0" @if($value == '0') selected @endif>{{__('dashboard.activation.in_active')}}</option>
        </select>
    </div>
