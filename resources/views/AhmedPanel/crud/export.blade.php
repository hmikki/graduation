    ...
    <table class="table borderless tbl_header">
        <tr>
            <td width="50%" class="text @if(app()->getLocale() == 'ar') text-right @else text-left @endif " align="right">
                <span>{{__('dashboard.count') .' '. count($Objects)}}</span><br>
                <span>{{__('dashboard.exported_by')}} : {{ auth('dashboard')->user()->name }}</span><br>
                <span>{{__('dashboard.exported_date')}} : {{ date('d-m-Y') }}</span>
            </td>
